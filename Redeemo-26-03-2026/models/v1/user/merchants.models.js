...
const getUser = async (req , res ) => {
    try {
        ...
        if(userData && userData.length > 0){
            return sendResponse(req, res, 200, 1, "User Details Fetch", { userData })
        }
...
const updateLocation = async (req , res ) => {
    try {
        ...
        if (!is_userExist) {
        return sendResponse(req, res, 404, 0, "User not found", {});
        }
        ...
        const [updateLocation] = await db.query("UPDATE tbl_user SET lat = ? , log = ? WHERE id = ? " , [lat , log , user_id] )
         if(updateLocation && updateLocation.affectedRows > 0){
            return sendResponse(req, res, 200, 1, "User Location Updated Successfully", {  })
        }
        return sendResponse(req, res, 200, 0, "Please Provide Currect Location", {  })

    } catch (error) {
       console.log(error)
       return sendResponse(req, res, 500, 0, "Internal Server Error", { err })
      }
}
...
limit 3;
    ...
    if(vouchers && vouchers.length > 0){
        return sendResponse(req, res, 200, 1, "Neares Voucher Fetched ", {vouchers});
    }
    return sendResponse(req, res, 200, 1, "Vouchers not Found ", {});
  } catch (error) {
    console.error(error);
    return sendResponse(req, res, 500, 0, "Internal Server Error", {error});
  }
...
const fetchCategories = async (req, res) => {
  try {
    ...
    if (categories.length === 0) {
      return sendResponse(req, res, 404, 0, "No categories found", {});
    }
    let count = categories.length
    return sendResponse(req, res, 200, 1, "Categories fetched successfully", {"Records Found" : count , categories });
  } catch (error) {
    console.error(error);
    return sendResponse(req, res, 500, 0, "Internal Server Error", error);
  }
...
`, [user[0].lat, user[0].log, user[0].lat])
      if(merchants[0]){
        return sendResponse(req, res, 200, 1, "Trending merchants fetched successfully", merchants);
      }
      return sendResponse(req, res, 404, 0, "No trending merchants found", {});
    } catch(error){
      console.log( error);
      return sendResponse(req, res, 500, 0, "Internal Server Error", error);
    }
...
`, [user[0].lat, user[0].log, user[0].lat])
      if(Caterymerchants[0]){
        return sendResponse(req, res, 200, 1, "Category Wise Trending merchants", Caterymerchants);
      }
      return sendResponse(req, res, 404, 0, "No merchants found", {});
  } catch (error) {
    console.log( error);
    return sendResponse(req, res, 500, 0, "Internal Server Error", error);
  }
...
const fetchAllNearByMerch = async (req , res) => {
  ...
  ORDER BY distance_km ASC;
      ...
      if(merchants[0]){
        return sendResponse(req, res, 200, 1, "NearBy merchants", merchants);
      }
      return sendResponse(req, res, 404, 0, "No merchants found", {});
  } catch (error) {
    console.log( error);
...
const toggleFavouriteMerchant = async (req, res) => {
    try {
        ...
        if (existing.length === 0) {
            ...
            return sendResponse(req, res, 200, 1, "Merchant Added To Favourite", { is_favourite: 1 })
        }
        ...
        return sendResponse(req, res, 200, 1, "Merchant Removed From Favourite", { is_favourite: 0 })
...
const toggleFavouriteVoucher = async (req, res) => {
    try {
        ...
        if (!voucher || voucher.length === 0) {
            return sendResponse(req, res, 200, 3, "Voucher not found", {})
        }
        ...
        if (existing.length === 0) {
            ...
            return sendResponse(req, res, 200, 1, "Voucher Added To Favourite", { is_favourite: 1 })
        }
        ...
        return sendResponse(req, res, 200, 1, "Voucher Removed From Favourite", { is_favourite: 0 })
...
const fetchVoucherDetails = async (req, res) => {
    try {
        ...
        if (!voucher_id) {
            return sendResponse(req, res, 400, 2, "voucher_id is required", {})
        }
        ...
        return sendResponse(req, res, 200, 1, "Voucher Details Fetched Successfully", { voucher: voucherData })
...
const redeemVoucher = async (req, res) => {
    try {
        ...
        if (!voucher || voucher.length === 0) {
            return sendResponse(req, res, 200, 3, "Voucher Not Found Or Expired", {})
        }
        ...
        if (existing && existing.length > 0) {
            return sendResponse(req, res, 200, 0, "You Have Already Redeemed This Voucher", { is_redeemed: 1 })
        }
        ...
        return sendResponse(req, res, 200, 1, "Voucher Redeemed Successfully", {
            is_redeemed: 1,
    ...
    try {
        ...
        if (existing && existing.length > 0) {
            ...
            if (!result || result.affectedRows !== 1) {
                return sendResponse(req, res, 500, 0, "Failed To Add Rating", {})
            }
        }
        ...
        return sendResponse(req, res, 200, 1, existing.length > 0 ? "Rating Updated Successfully" : "Rating Added Successfully", {
            merchant_id,
...
const fetchRatingListing = async (req, res) => {
    try {
        ...
        if (!ratings || ratings.length === 0) {
            return sendResponse(req, res, 200, 3, "No ratings found", {})
        }

        return sendResponse(req, res, 200, 1, "Merchant ratings fetched successfully", { ratings })
    } catch (error) {
        console.log(error)
        return sendResponse(req, res, 500, 0, "Internal Server Error", { error })
    }
...
const fetchFavourites = async (req, res) => {
    try {
        ...
        if (type === 'voucher') {
            ...
            if (total === 0) {
                return sendResponse(req, res, 200, 3, "No Favourite Voucher Found", {})
            }
            ...
            return sendResponse(req, res, 200, 1, "Favourite Vouchers Fetched Successfully", {
        ...
        if (total === 0) {
            return sendResponse(req, res, 200, 3, "No Favourite Merchant Found", {})
        }
        ...
        return sendResponse(req, res, 200, 1, "Favourite Merchants Fetched Successfully", {
...
const deleteRating = async (req , res) => {
  try {
    ...
    if(!rating_id){
      return sendResponse(req, res, 200 , 0, "Please Provide Rating To Delete", { error })
    }
    
    const [existing] = await db.query(`SELECT * FROM tbl_merchant_rating WHERE id = ? AND is_active = 1` , [rating_id])
    if(existing.length === 0){
      return sendResponse(req, res, 200 , 0 , "No Rating Data found", {  })
    }
    ...
    if(deleteRating && deleteRating.affectedRows > 0){
      return sendResponse(req, res, 200 , 1 , "Rating Deleted Successfully", ratingData)
    }
    return sendResponse(req, res, 200 , 1 , "You cannot Delete Other Users Rating", {  })
  } catch (error) {
    console.log(error)
    return sendResponse(req, res, 500, 0, "Internal Server Error", { error })
  }
...
const fetchTrendingMerchants = async (req, res) => {
    try {
        ...
        if (!merchantsRaw || merchantsRaw.length === 0) {
            return sendResponse(req, res, 200, 3, "No Trending Merchant Found", {})
        }
        const merchants = merchantsRaw.map((m) => ({
            id: m.id,
            admin_id: m.admin_id,
        ...
        return sendResponse(req, res, 200, 1, `${total} Merchants Available`, {
